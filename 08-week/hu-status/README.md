...
| HU-000-016 | Carried over — As the team, we want DOCS's API contracts to reflect the real auth model (HS512/24h, real roles), so `07-api` stops contradicting `AuthService` | doing, progressed twice (approved by the automated review at [`3034740`](https://github.com/code-corhuila/barber-saas-docs/commit/3034740), re-review pending on the follow-up fix) | DOCS [PR #16](https://github.com/code-corhuila/barber-saas-docs/pull/16) (retitled to reflect SPEC-004+SPEC-008), branch `docs/004-align-api-contracts`: rebased onto `main`, added `07-api/open-questions.md` (3 real contract gaps with owner + closing criterion) in [`47bbe9b`](https://github.com/code-corhuila/barber-saas-docs/commit/47bbe9b), answered the reviewer's 3 recommendations on that commit, then fixed a real bug the *second* automated review caught — `notification-service.yaml`'s planned server URLs contradicted `guidelines.md`'s own versioning rule and `auth-service.yaml`'s pattern — in [`9baf72c`](https://github.com/code-corhuila/barber-saas-docs/commit/9baf72c), all documented in the [PR comment thread](https://github.com/code-corhuila/barber-saas-docs/pull/16#issuecomment-5815005803) |
...
- **DOCS PR backlog is growing, not WIP-limited**: 5 PRs open (`#15`, `#16`, `#17`, `#18`, `#20`), 0 merged this week — each needs CODEOWNERS/teacher (`ariel5253`) approval to land, and none has yet. PR #16 specifically is now `MERGEABLE` with today's review recommendations answered, but `mergeStateStatus` is still `BLOCKED` pending that approval — closing the individual PR's own gaps doesn't unblock the queue by itself.
...
- [x] Conventional Commits - `type(scope): summary` — all four commits this week (`72c622f`, `494c563`, `47bbe9b`, `3034740`) follow `type(scope): summary`, and the two DOCS commits carry `Spec: SPEC-008`.
...
- DOCS, SPEC-004 API contracts PR (open since Week 07, updated today): https://github.com/code-corhuila/barber-saas-docs/pull/16
- DOCS, SPEC-008 individual contribution (`open-questions.md`): https://github.com/code-corhuila/barber-saas-docs/commit/47bbe9b
- DOCS, PR #16 review-recommendation responses: https://github.com/code-corhuila/barber-saas-docs/commit/3034740 and https://github.com/code-corhuila/barber-saas-docs/pull/16#issuecomment-5814896761
- DOCS, second-round bug fix + follow-up responses: https://github.com/code-corhuila/barber-saas-docs/commit/9baf72c and https://github.com/code-corhuila/barber-saas-docs/pull/16#issuecomment-5815005803
